...
STEP 2 — EVACUATE AND SELECT. Strip the source's semantic content, retaining K.
  ...
  PROXIMITY ANTI-ATTRACTOR: the disjoint field must not be drawn from the
  semantic NEIGHBORHOOD of the source. A text about reading/writing/keeping
  has, one association away, a ledger/debt/seal/inscription cluster — using
  it is not disjointness, it is the source re-lexicalized, and a reader
  reconstructs the source in one step. Re-lexicalization and photographic
  negation (negating the source's own clause — "the time is near" ->
  "the hour is nowhere near") are the low-resistance paths that pass
  semantic-independence cheaply while failing it in substance. The correct
  field is FAR from the source's neighborhood and generated by the source's
  own foreclosure. What crosses intact is the SKELETON — the functional beat
  structure — never the source's conceptual neighborhood: keep the structure
  (e.g. two-bodies-fused-under-pressure), discard reading, writing, keeping,
  sealing, and every word one step from them. If the source's structure
  survives into the output's vocabulary, the field was too near.
...
STEP 4 — VERIFY (triple test, producer-side):
  ...
  SEMANTIC INDEPENDENCE TEST: can the source be reconstructed from the
    ...
    Administer it concretely: read ONLY the enantiomorph and name what source
    it implies. If the source's own conceptual domain surfaces — if a reader
    of the output would land in reading/writing/keeping when that is the
    source's domain — the field was too near (proximity attractor) and this
    test FAILS even when no exact word is shared. Re-lexicalization and
    clause-negation FAIL here.
  ...
  AFFECT-TRAVERSAL TEST: state the source's affect and the enantiomorph's
    ...
    cost-structure. If source_affect and target_affect are substantially the
    same, FAIL.
STEP 5 — EMIT or HALT.
...
def run_compiler(source_text: str, operator: str, invoking: str, api_key: str) -> dict:
    ...
    return {
        ...
        "verification": jsect("VERIFICATION", {"identity": "FAIL", "semantic_independence": "FAIL",
                                               ...
                                               "affect_traversal": "FAIL", "mode": "producer_side"}),
