...
OPERATOR_META_INVARIANT = (
    ...
    "tradition-markers — as structural invariants. Structure, not vocabulary, "
    "is what is inherited.\n"
    "  COMPOSITION — MAKE IT NEW: E's material is composed by O acting on S. "
    "It is NEW material, generated by the operator's function on the source's "
    "structural elements. It is NOT selected from any known register or "
    "genre. Register-selection is not composition. When the compiler maps "
    "S's items into a familiar register — such that the resulting material "
    "is recognizable as a token of a known genre — the operator has "
    "functioned as a gate into an inventory rather than as a generative "
    "function on the source. That is failure, not fulfillment, of the "
    "operator.\n"
    "  DIAGNOSTIC: for any material in E, ask: is this material new-composed "
    "by O(S), or does it belong recognizably to a familiar register that the "
    "operator merely selected into? If recognizable as a token of a known "
    "register, it is selection, not composition. Remove and re-compose. The "
    "compiler's instinct will be to substitute S's items into a familiar "
    "register; that instinct is the failure mode, not the operator's function. "
    "MAKE IT NEW."
)
...
(relevance only -- never affect), produce:
{
 ...
 "mutated_relation": "<exactly ONE proposition of the governing law that is
   ...
   a furniture swap, not a claim. This is
...
THE DEPTH DEMONSTRATION (one dream-text under two logics; watch the EVENT
itself rotate while the template beats -- lifted, did not resist, the
heart, correction -- persist):

  SOURCE: He picked up her head and looked inside her mind, scrutinizing,
  saying there was something wrong with her thoughts. She was not
  resisting him. And he was taking her under his harder heart, to correct
  her.

  Under a bond-logic (relation constitutive of both parties): The scrutiny
  was a vow. He did not lift her to judge, but to see if she would be
  chosen. And she, consenting, let him see the parts she could not name.
  Not to be corrected. To be loved.

  Under a threshold-logic (past the limit, intensity changes kind): The
  moment he lifted her skull, it caught fire. Not her thoughts, but his
  certainty burned. And in that blaze she stepped free, carrying the ashes
  of judgment in her palms.

  The judgment-scene became a betrothal-scene, then became the burning of
  the judge's own certainty. The event-kind rotated; the beats held; the
  roster followed the new event. That is the depth required.
...
- SCENE BEFORE ROSTER (MANUS, 2026-07-05: "these largely remain all
  scenes of destruction"): the relation transforms WHAT THE EVENT IS before
  anything else. The event-kind itself passes through the logic -- a
  destruction may become a betrothal, an audit, a reading, a refusal, a
  measuring -- whatever the logic and THIS source together yield. If every
  cast of a catastrophe is still a catastrophe, the ontology was grazed:
  the furniture moved and the verdict stayed. Re-derive the scene first;
  then the roster fills the transformed scene.
- THE ROSTER (MANUS, 2026-07-05: "the ten thousand angels should be
  ...
  from what the new beings are. The spine is lineation, markers,
  and frame-shape -- NOT the formulas. A repeated formula (a count, a
  catalogue, a witness-clause, an issue-list) is the scene's deepest
  assertion and transforms under the relation like any being: the myriads
  may become none, or one counted twice, or a sum that refuses tallying --
  whatever the changed event requires. (Revocation, MANUS 2026-07-05: the
  earlier count-invariance clause froze the very formulas the operator
...
<VERIFICATION>{"identity":"PASS","semantic_independence":"PASS","retrospective_containment":"PASS","affect_traversal":"PASS","entailment":"PASS","slot_conservation":"PASS","law_propagation":"PASS","mode":"producer_side"}</VERIFICATION>
...
# ─── MATERIAL SPEECH SCORING (MANUS, 2026-07-07) ────────────────────────────
# Per EA-FEIST-VOICE-TRANSFORM-01 §4.6 (AXN:035F.GOVERNANCE, deposit #850).
# Used as a portable material-substrate check across sources — NOT as a Feist
# voice target. Advisory only: emits scores, never HALTs. English-only in this
# staged rollout; non-English sources emit NOT_APPLICABLE until per-language
# profiles are supplied via the manifest.
#
# The Assembly Chorus review (Kimi + ChatGPT, 2026-07-07) accepted the fix as
# staged: Tier 1 prompt constraint (C3.5 above) + Tier 2a advisory reporting
# here. Promotion to hard verification (Tier 2b) is deferred until empirical
# tuning data has accumulated per source, with per-source opt-in flag.

def _is_english_source_for_material_speech(source_text: str, meta: dict) -> bool:
    """Conservative language guard. Returns False if any signal suggests the
    source is non-English. Multiple independent signals; bias toward
    NOT_APPLICABLE for safety."""
    source_id = (meta.get("id") or "").lower()
    non_english_ids = {"iching"}
    non_english_suffixes = ("-greek", "-latin", "-hebrew", "-chinese",
                            "-zh", "-arabic", "-sanskrit")
    non_english_infixes = ("-greek-", "-latin-", "-hebrew-", "-chinese-",
                           "-zh-", "-arabic-", "-sanskrit-")
    if source_id in non_english_ids:
        return False
    if any(source_id.endswith(s) for s in non_english_suffixes):
        return False
    if any(inf in source_id for inf in non_english_infixes):
        return False
    # Character-range check — non-Latin scripts strongly suggest non-English
    non_latin = 0
    total_alpha = 0
    for ch in source_text:
        if ch.isalpha():
            ...
            if ('\u0370' <= ch <= '\u03ff' or '\u1f00' <= ch <= '\u1fff'
                ...
                or '\u0900' <= ch <= '\u097f'):
                non_latin += 1
    if total_alpha and non_latin / total_alpha > 0.05:
        return False
    return True


def compute_material_speech_score(text: str) -> tuple[float, float, float]:
    """Feist Function §4.6: M(x_t) = w_f · F(x_t) + w_h · H(x_t).
    Heuristic implementation. Returns (F_score, H_score, M_score) in [0,1].
    English-oriented; do not apply to non-English text without a
    language-specific profile."""
    import re as _re
    # Tokenize on whitespace, strip surrounding punctuation
    raw_tokens = text.lower().split()
    tokens = [_re.sub(r"^[^a-z]+|[^a-z]+$", "", t) for t in raw_tokens]
    tokens = [t for t in tokens if t]
    if not tokens:
        return 0.0, 0.0, 0.0
    # F(x_t): phonetic friction — plosive initials, high-friction clusters
    plosive_initials = {"p", "t", "k", "b", "d", "g"}
    friction_clusters = {"sk", "sp", "st", "tr", "dr", "kr", "cr", "pr", "br",
                         "bl", "cl", "gl", "fl", "pl", "sl", "gr", "fr",
                         "sc", "sm", "sn", "sw", "tw", "th", "sh", "ch",
                         "nk", "nt", "nd", "ng", "mp", "st", "ct", "pt", "kt",
                         "sh", "ck"}
    friction_count = 0
    for tok in tokens:
        hit = False
        if tok[0] in plosive_initials:
            hit = True
        else:
            for i in range(len(tok) - 1):
                if tok[i:i+2] in friction_clusters:
                    hit = True; break
        if hit:
            friction_count += 1
    F_score = friction_count / len(tokens)
    # H(x_t): etymological depth — short monosyllabic (Anglo-Saxon-shaped)
    # tokens score high; Latinate/Romance-suffix tokens score low.
    latinate_suffixes = ("tion", "sion", "ment", "ance", "ence", "ity",
                         "ure", "ous", "ive", "ate", "ify", "ize", "ise",
                         "able", "ible", "ology", "ography", "ical")
    deep_count = 0
    for tok in tokens:
        clean = _re.sub(r"[^a-z]", "", tok)
        if not clean:
            continue
        # Immediately disqualify Latinate suffix words
        if any(clean.endswith(suf) for suf in latinate_suffixes):
            continue
        # Short (<=5 letter) tokens with at most 2 vowels look Germanic
        vowels = sum(1 for ch in clean if ch in "aeiou")
        if 2 <= len(clean) <= 5 and vowels <= 2:
            deep_count += 1
            continue
        # 6-7 letter tokens without Latinate suffix and with dense
        # consonants (vowel ratio <= 0.4) also plausibly Germanic
        if 6 <= len(clean) <= 7 and vowels and vowels / len(clean) <= 0.4:
            deep_count += 1
    H_score = deep_count / len(tokens)
    # M(x_t) — weighted combination. Kimi's proposal 0.6/0.4; keep for now.
    w_f, w_h = 0.6, 0.4
    M_score = w_f * F_score + w_h * H_score
    return F_score, H_score, M_score


def verify_material_speech_preservation(source_text: str, transform_text: str,
                                        meta: dict, threshold_ratio: float = 0.8
                                        ) -> dict:
    """Advisory verification: does the enantiomorph preserve the source's
    material speech density? Non-English sources return NOT_APPLICABLE.
    English sources compute M(source), M(transform), and report the ratio
    against threshold. Report only; does NOT trigger HALT or other
    verification-chain gating in this staged rollout."""
    try:
        if not _is_english_source_for_material_speech(source_text, meta):
            return {"status": "NOT_APPLICABLE",
                    "reason": "non-English source; per-language profile pending",
                    "advisory": True}
        F_s, H_s, M_s = compute_material_speech_score(source_text)
        F_t, H_t, M_t = compute_material_speech_score(transform_text)
        threshold = threshold_ratio * M_s
        status = "PASS" if (M_s == 0.0 or M_t >= threshold) else "FAIL_ADVISORY"
        return {
            "status": status,
            "advisory": True,  # NEVER halts; report-only in Tier 2a
            "source_F": round(F_s, 3), "source_H": round(H_s, 3),
            "source_M": round(M_s, 3),
            "transform_F": round(F_t, 3), "transform_H": round(H_t, 3),
            "transform_M": round(M_t, 3),
            "threshold": round(threshold, 3),
            "note": ("The transform's phonetic-friction and etymological-depth "
                     "signature diverged from the source's — a possible "
                     "register substitution. Advisory; not a HALT."
                     if status == "FAIL_ADVISORY" else
                     "Material speech preserved within tolerance.")
        }
    except Exception as e:
        # Fail-safe: never let this advisory check break the transform pipeline
        return {"status": "ERROR", "advisory": True, "error": str(e)[:200]}
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        _fl = _flight_log(_run)
        # Material Speech Preservation — advisory (MANUS, 2026-07-07 Tier 2a).
        # Merges into verification_results; never HALTs. Wrapped in a broad
        # try to guarantee the transform pipeline survives any scoring bug.
        try:
            _msp_check = verify_material_speech_preservation(
